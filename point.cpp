...
Point operator+ (Point& p1, Point& p2) {
  ...
  unsigned majorDim = (p1.coord.size() > p2.coord.size() ? p1.coord.size() : p2.coord.size());
  Point sum(majorDim); // Create a variable coordinates 0s with greater dimension

  for (unsigned i=0; i < majorDim; i++) {
...
Point operator* (double m1, Point& p1) {
  ...
  for (unsigned i=0; i < p1.coord.size(); i++)
...
ostream& operator<< (ostream&out, Point& p) {
    out << "(";
    for (unsigned i = 0; i < p.coord.size() - 1; ++i)
        out <<  p.coord[i] << ", ";
